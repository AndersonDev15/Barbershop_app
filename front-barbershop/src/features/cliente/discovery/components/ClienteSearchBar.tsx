      ...
      <div
        ...
        ${
          isNavbar
            ...
            : "max-w-3xl mx-auto bg-surface-container-low p-1.5 md:p-2 rounded-full border border-outline-variant/10 shadow-2xl focus-within:border-primary/30"
        }
      `}
      >
        <div
          className={`flex-1 flex items-center ${isNavbar ? "px-3 md:px-4" : "px-4 md:px-6"}`}
        >
          {isLoading ? (
            <span
              className={`material-symbols-outlined text-primary animate-spin ${isNavbar ? "text-base md:text-lg mr-2" : "text-lg md:text-xl mr-3 md:mr-4"}`}
            >
              progress_activity
            </span>
          ) : (
            <span
              ...
              ${isNavbar ? "text-base md:text-lg mr-2" : "text-lg md:text-xl mr-3 md:mr-4"}
            `}
            >
              search
            </span>
          ...
          <input
            className={`
              ...
              ${isNavbar ? "text-xs md:text-sm py-1.5" : "text-sm md:text-base py-2.5 md:py-3"}
            `}
            placeholder={
              isNavbar
                ? "Buscar..."
